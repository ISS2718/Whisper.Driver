...
#define PORTA 8008
#define NOME_MODULO "TCP Server"
#define CONN_MAX 16

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Hugo H. Nakamura");
...
int enviar(struct socket *localSocket, const char * mensagem, const size_t tam, unsigned long flags){
    struct msghdr msg;
    struct kvec vec;
...
int conectar(void){
    ...
    strcat(reply, "HOLA\n");
    enviar(socket, reply, strlen(reply), MSG_DONTWAIT);
